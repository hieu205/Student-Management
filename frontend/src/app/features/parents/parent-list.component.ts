    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
      ...
      <div class="p-6 border-b border-gray-100 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-800">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          ...
          <button *ngIf="authService.hasPermission('parent:create')"
            (click)="openAddModal()"
      ...
      <div *ngIf="authService.hasPermission('parent:read')">
        <!-- Table View -->
            ...
            <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-100 dark:divide-slate-700">
              ...
              <tr *ngFor="let parent of parents()" class="hover:bg-blue-50 dark:bg-blue-900/20 dark:hover:bg-slate-700/60 transition-all duration-300 group">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium action-dropdown-container relative align-middle">
                  <button *ngIf="authService.hasPermission('parent:update') || authService.hasPermission('parent:delete')" (click)="toggleDropdown(parent.id, $event)" class="text-gray-400 hover:text-blue-600 dark:text-blue-400 p-2 rounded-full hover:bg-blue-50 dark:bg-blue-900/20 dark:hover:bg-slate-700 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-100">
                  ...
                  <div *ngIf="activeDropdown() === parent.id" class="absolute right-8 top-10 w-36 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-100 dark:border-slate-700 z-50 animate-fade-in-up overflow-hidden">
                    <div class="py-1">
                      <button *ngIf="authService.hasPermission('parent:update')" (click)="openEditModal(parent.id)" class="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 hover:text-orange-500 flex items-center transition-colors cursor-pointer">
                      ...
                      <button *ngIf="authService.hasPermission('parent:delete')" (click)="confirmDelete(parent.id)" class="w-full text-left px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 flex items-center transition-colors">
          ...
          <div *ngIf="!isLoading() && parents().length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <div *ngFor="let parent of parents()" class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-6 flex flex-col items-center text-center hover:shadow-md transition-shadow relative">
              <div class="absolute top-4 right-4 action-dropdown-container">
                <button *ngIf="authService.hasPermission('parent:update') || authService.hasPermission('parent:delete')" (click)="toggleDropdown(parent.id, $event)" class="text-gray-400 hover:text-blue-600 dark:text-blue-400 focus:outline-none">
                ...
                <div *ngIf="activeDropdown() === parent.id" class="absolute right-0 top-6 w-36 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-100 dark:border-slate-700 z-50 animate-fade-in-up text-left overflow-hidden">
                  <div class="py-1">
                    <button *ngIf="authService.hasPermission('parent:update')" (click)="openEditModal(parent.id)" class="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 flex items-center">Sửa</button>
                    <button *ngIf="authService.hasPermission('parent:delete')" (click)="confirmDelete(parent.id)" class="w-full text-left px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 flex items-center">Xóa</button>
                  </div>
                </div>
              </div>
              ...
              <button *ngIf="authService.hasPermission('parent:update')" (click)="openEditModal(parent.id)" class="mt-auto px-5 py-2 w-full rounded-full border border-blue-600 dark:border-blue-400 text-blue-600 dark:text-blue-400 hover:bg-blue-600 hover:text-white dark:hover:bg-blue-400 dark:hover:text-slate-900 transition-colors font-medium">Chỉnh sửa</button>
            </div>
          </div>
        </div>
      </div>

      <div *ngIf="!authService.hasPermission('parent:read')" class="p-12 text-center bg-gray-50 dark:bg-slate-900/30 min-h-[400px] flex flex-col justify-center items-center">
        <div class="bg-white dark:bg-slate-800 rounded-full w-20 h-20 flex items-center justify-center mx-auto mb-4 shadow-sm border border-gray-100 dark:border-slate-700">
        ...
        <p class="text-gray-500 dark:text-slate-400 max-w-md mx-auto">Tài khoản của bạn không được cấp quyền xem danh sách phụ huynh. Bạn chỉ có thể thực hiện thao tác Thêm mới nếu được phân quyền.</p>
      </div>
...
export class ParentListComponent implements OnInit {
  ...
  loadParents() {
    if (!this.authService.hasPermission('parent:read')) return;
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          if (err.status === 401) {
            ...
            alert('Bạn không có quyền xem danh sách Phụ huynh!');
          } else {
