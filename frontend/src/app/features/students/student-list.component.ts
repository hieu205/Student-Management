...
@Component({
  ...
  template: `
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
      ...
      <div *ngIf="authService.hasPermission('student:read')">
        <!-- Table View -->
        <div class="overflow-x-auto" *ngIf="viewMode() === 'list'">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
            <thead class="bg-gray-50 dark:bg-slate-900/50">
              <tr>
                <!-- Sortable Headers -->
                <th scope="col" (click)="toggleSort('studentCode')" class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:text-blue-600 dark:text-blue-400 transition-colors group select-none whitespace-nowrap">
                ...
                <th scope="col" (click)="toggleSort('className')" class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:text-blue-600 dark:text-blue-400 transition-colors group select-none whitespace-nowrap">
                  <div class="flex items-center gap-1">
                    Lớp
                ...
                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap">Ngày sinh</th>
                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap">Giới tính</th>
                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap">Phụ huynh đã gán</th>
                  ...
                  <div *ngIf="student.parents && student.parents.length > 0" class="flex flex-col divide-y divide-gray-100 dark:divide-slate-700 w-full">
                    <div *ngFor="let parent of (expandedRows().has(student.id) ? student.parents : (student.parents | slice:0:2))" class="flex items-center gap-3 py-2.5 animate-fade-in-up w-full">
                      ...
                      <div class="flex flex-col overflow-hidden">
                        ...
                        <span class="text-xs text-gray-500 dark:text-slate-400 mt-0.5 truncate">{{ parent.relationshipType === 'Father' ? 'Bố' : (parent.relationshipType === 'Mother' ? 'Mẹ' : (parent.relationshipType === 'Guardian' ? 'Người giám hộ' : (parent.relationshipType || 'Phụ huynh'))) }} - {{ parent.phoneNumber }}</span>
                      </div>
                    </div>
            ...
            <div *ngFor="let student of students()" class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-6 flex flex-col items-center text-center hover:shadow-md transition-shadow relative">
              ...
              <div class="w-full space-y-2 text-sm text-left border-t border-gray-100 dark:border-slate-700 pt-4 mb-5">
                ...
                <div class="flex justify-between items-center gap-2">
                  ...
                  <span class="font-medium text-gray-700 dark:text-slate-200 truncate" [title]="formatAddress(student.address)">{{ formatAddress(student.address) || '—' }}</span>
                </div>
              </div>
      ...
      <div *ngIf="!authService.hasPermission('student:read')" class="p-12 text-center bg-gray-50 dark:bg-slate-900/30 min-h-[400px] flex flex-col justify-center items-center">
        ...
        <p class="text-gray-500 dark:text-slate-400 max-w-md mx-auto">Tài khoản của bạn không được cấp quyền xem danh sách và chi tiết học sinh. Bạn chỉ có thể thực hiện thao tác Thêm mới nếu được phân quyền.</p>
      </div>
...
export class StudentListComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.authService.hasPermission('student:read')) {
      this.loadStudents();
    }
  }

  loadStudents() {
    if (!this.authService.hasPermission('student:read')) return;
