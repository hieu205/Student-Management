...
@Component({
  ...
  template: `
    ...
    <div *ngIf="showDeleteConfirm()" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
      <div class="fixed inset-0 bg-gray-900/75 dark:bg-slate-900/80 backdrop-blur-sm transition-opacity" (click)="closeDeleteConfirm()"></div>
      <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-xl max-w-md w-full overflow-hidden transform transition-all border border-gray-100 dark:border-slate-700">
        <div class="p-6">
          ...
          <h3 class="text-lg font-bold text-gray-900 dark:text-slate-100 text-center mb-2">Xóa Nhóm Quyền</h3>
          <p class="text-gray-500 dark:text-slate-400 text-center text-sm">
            Bạn có chắc chắn muốn xóa nhóm quyền <span class="font-bold text-gray-800 dark:text-slate-200">"{{ roleToDelete()?.name }}"</span> không? Hành động này không thể hoàn tác.
          </p>
        </div>
        <div class="px-6 py-4 bg-gray-50 dark:bg-slate-700 border-t border-gray-100 dark:border-slate-700 flex justify-end gap-3">
          <button (click)="closeDeleteConfirm()" class="px-4 py-2 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-lg text-sm font-medium text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
            Hủy bỏ
          </button>
          <button (click)="confirmDeleteRole()" [disabled]="isDeleting()" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium transition-colors shadow-sm disabled:opacity-50 flex items-center">
